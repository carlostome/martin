...
findClause m = do
  -- REMARK: HERE AT THIS POINT I REPLACED getImportedSignature with getSignature,
  -- because our way of type checking things apparently puts stuff into the normal signature,
  -- not the imported one. The rest of this file is still the same as in the Agda source.
  sig <- getSignature
  let res = do
